...
- Instantiation with optional size and optional position: def __inti__(self,
  size=0, position=(0, 0))
  * size must be an integer, otherwise raise a TypeError exception with the
    message size must be an integer
...
class Square:
    ...
    def __repr__(self):
        """
        The repr method prints the string of the a Square instance should have
        the same behavior as my_print().

        The goal is to create a string instead of using print().

        Return all of the string.
        """
        string = ""
        if self.__size == 0:
            return string
