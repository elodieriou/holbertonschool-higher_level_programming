...
```
    A Python program terminates as soon as it encounters an unhandled error. There two classes of error: Syntax errors and Logical error as Exceptions.
    __Python Syntax errors:__ there are error caused by not following the proper structure of the langua
ge is called 'syntax error or parsing error'. For example missing a colon ':' in the if statement.
    __Python Exceptions:__ there are error that occured after passing the syntax test. These exceptions
occur when we try to open a file 'FileNotFoundError', try to divide a number by zero 'ZeroDivisionError'
